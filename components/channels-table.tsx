...
import { Youtube, MoreHorizontal, ExternalLink, Trash2, LayoutGrid, List, AlignJustify } from "lucide-react"
...
export function ChannelsTable({ groupId }: ChannelsTableProps) {
  ...
  const sortedChannels = [...filteredChannels].sort((a, b) => {
    switch (sortOrder) {
      case "subscribers-desc":
        return (
          Number.parseInt(b.subscribers.replace(/[^\d]/g, "")) - Number.parseInt(a.subscribers.replace(/[^\d]/g, ""))
        )
      case "subscribers-asc":
        return (
          Number.parseInt(a.subscribers.replace(/[^\d]/g, "")) - Number.parseInt(b.subscribers.replace(/[^\d]/g, ""))
        )
      case "name-asc":
                ...
                <TableCell className="text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem className="text-destructive" onClick={() => handleRemoveChannel(channel.id)}>
          ...
          <Card key={channel.id}>
            <CardContent className="p-4">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-xs text-muted-foreground mb-2 truncate w-full">{channel.url}</p>
                <div className="flex gap-2 mb-3">
                  <Badge variant="secondary">{channel.subscribers}</Badge>
                  <Badge variant="outline">{channel.videos} videos</Badge>
                </div>
                <p className="text-sm mb-4 line-clamp-2">{channel.description}</p>
  ...
  const renderCompactView = () => (
    <div className="space-y-2">
      {sortedChannels.length === 0 ? (
        <div className="h-24 flex items-center justify-center text-center border rounded-md">No channels found.</div>
      ) : (
        sortedChannels.map((channel) => (
          <div
            key={channel.id}
