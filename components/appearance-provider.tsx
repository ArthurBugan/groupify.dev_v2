...
export function AppearanceProvider({ children, initialSettings }: AppearanceProviderProps) {
  ...
  React.useEffect(() => {
    ...
    const accentColorValue = isDark ? accentColors[settings.accentColor].dark : accentColors[settings.accentColor].light
  ...
  const contextValue: AppearanceContextType = {
    ...
    setCompactMode: (compact) => setSettings((prev) => ({ ...prev, compactMode: compact })),
    setAnimations: (enabled) => setSettings((prev) => ({ ...prev, animations: enabled })),
  }
